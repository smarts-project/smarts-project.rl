...
class ObsWrapper():
    ...
    def observation(self, obs):
        ...
        if (self.target_lane_index == None) or (self.target_lane_index not in all_lane_indeces):
        ...
        if   self.target_lane_index < lane_index: EnvInfo_is_target[0] = 1.0
        elif self.target_lane_index > lane_index: EnvInfo_is_target[2] = 1.0
        ...
        wrapped_obs = np.concatenate([
            self.preserved_info.wrapped_obs,
            self.preserved_info.classifier_input
        ], -1)
...
class EnvWrapper():
    ...
    def pack_action(self, action):
        ...
        for neighbor in self.obs_wrapper.preserved_info.raw_obs.neighborhood_vehicle_states:
        ...
        # ? Rule-based lane changing condition 3
            ...
            if self.obs_wrapper.preserved_info.lane_index == self.obs_wrapper.target_lane_index \
                and action in [7, 8, 9]:
                if goal_lane[2] and self.is_in_safe_box(True): action = 2
                elif goal_lane[0] and self.is_in_safe_box(False): action = 5
        ...
        st = np.where(all_lane_indeces == self.obs_wrapper.target_lane_index)[0][0].item()
