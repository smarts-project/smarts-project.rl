...
class EnvWrapper():
    def __init__(self, agent_id, model):
        ...
        self.observation_space = gym.spaces.Box(-1E10, 1E10, (19 + 5 * self._N_NEIGHBORS,))
    ...
    def cal_goal_lane(self, raw_obs):
        ...
        if len(self.all_lanes) == 0 or \
                not hasattr(raw_obs.ego_vehicle_state.mission.goal, "position"): return goal_lane
        ...
        if cos_thetas.max() > 1 - 0.0001:
            goal_lane[np.clip(
                2 + cos_thetas.argmax() - self.lane_index, 0, 4
            )] = 1.0
        return goal_lane
    ...
    def rule_based_action(self, wps_bias):
        ...
        if self.agent_tag == 'unknown':
            try:
                ...
                if hasattr(raw_obs.ego_vehicle_state.mission.goal, "position"):
                    ...
                    goal_str = 'x_' + str(round(goal_pos[0], 1)) + '_y_' + str(round(goal_pos[1], 1))
                else:
                    goal_str = 'no_goal'
        ...
        try:
            if len(raw_obs.waypoint_paths) > 1:
                ...
                if len(path_dheadings) > 1:
                    ...
                    if max_dh_end < 1e-4 and max_dist_to_ego < 1:  # todo 统计50m内且位于不可达车道上的车辆
                        for lane_idx_in_list, path_wps in enumerate(raw_obs.waypoint_paths):
                            ...
                            if max_dheading < 45 / 180 * math.pi:  # todo O(n2)寻找小于pi的最大角度
                                path_dheadings.append((lane_idx_in_list, abs_dheadings))
                        ...
                        for nb in raw_obs.neighborhood_vehicle_states:
                            if np.linalg.norm(curr_pos - nb.position[:2]) and nb.lane_id not in reachable_lane_ids:
                                if nb.id[:5] != 'Agent':
        ...
        if self.near_goal and self.straight_tag and self.lane_index == self.target_lane_index and s_dist_from_center < \
            ...
            if self.changing_lane_while_merging or not self.other_agent_nearby:
                changing_lane = True
        if self.near_goal and self.straight_tag and changing_lane and (
        ...
        # TODO: Call the agent
        if self.agent_ignore_collision_steps > 0 or self.novice_steps > 0 or self.distance_travelled < 5 or collision_from_behind_tag:
            model_action = 1
        ...
        if self.ignore_collision_steps > 0:
            ...
            for key_nb_id in self.blocking_car_ids:
                ...
                for nb in raw_obs.neighborhood_vehicle_states:
                    if nb.id == key_nb_id:
                        if self.near_goal and self.straight_tag and changing_lane and nb.lane_index != self.target_lane_index:
                            if self.changing_lane_while_merging or not self.other_agent_nearby:
                                continue
                        ...
                        if cos_theta < math.cos(math.pi * 5 / 6):  # 侧后方碰撞不能停车
                            continue
                        if collision_forecast(raw_obs.ego_vehicle_state, nb, w_left=self.left_width, l_front=l_front,
                                              w_right=self.right_width, steps=forecast_steps):
                ...
                for neighbor in raw_obs.neighborhood_vehicle_states:  # 圆形范围检测会导致车辆卡住后面的车 所以暂时使用矩形
                    ...
                    if neighbor.id[:5] == 'Agent':
                        ...
                        if wps_bias[self.agent_id] < wps_bias[neighbor_agent_id]:
                            ...
                            continue  # 车辆序号大的让小的
                    if self.near_goal and self.straight_tag and changing_lane and neighbor.lane_index != self.target_lane_index:
                        if self.changing_lane_while_merging or not self.other_agent_nearby:
                            continue
                    if collision_forecast(raw_obs.ego_vehicle_state, neighbor, l_front=l_front, w_left=0.001,
                                          w_right=0.001,
            ...
            for neighbor in raw_obs.neighborhood_vehicle_states:
                ...
                if self.near_goal and self.straight_tag and changing_lane and neighbor.lane_index != self.target_lane_index:
                    if self.changing_lane_while_merging or not self.other_agent_nearby:
                        continue
                if collision_forecast(raw_obs.ego_vehicle_state, neighbor, l_front=l_front, w_left=self.left_width,
        ...
        for neighbor in raw_obs.neighborhood_vehicle_states:  # 计算附近3车道的预期速度
            ...
            if self.lane_index - 1 <= neighbor.lane_index <= self.lane_index + 1 and \
                    ...
                    neighbor.lane_id in reachable_lane_ids:  # todo 用relative pos和curr road heading判定车辆相对位置
                ...
                elif not changing_lane and neighbor_dists < right_dist_min and neighbor.lane_index == self.lane_index - 1 and self.lane_index > 0:
                    right_dist_min = neighbor_dists
                    right_exp_speed = min(neighbor.speed, right_exp_speed)
                elif not changing_lane and neighbor_dists < left_dist_min and neighbor.lane_index == self.lane_index + 1 and self.lane_index < self.n_lanes - 1:
        ...
        if not changing_lane:
            ...
            change_lane2right_ver_threshold = change_lane2left_ver_threshold = change_lane_ver_threshold = 1
            ...
            if hasattr(raw_obs.ego_vehicle_state.mission.goal, "position"):
                ...
                if dist_to_goal < change_to_goal_lane_dist_threshold:
                    ...
                    if lane_index_of_goal is not None and lane_index_of_goal != self.lane_index:
                        decrease_v_threshold = 100 / (dist_to_goal + 1e-5)  # 还有10m时克服10m/s的速度差
                        if self.straight_tag:
                            if self.changing_lane_while_merging or not self.other_agent_nearby:
                    ...
                    if lane_index_of_goal is not None and lane_index_of_goal <= self.lane_index:  # 序号由右至左排列，goal在当前车道右侧
                        change_lane2left_ver_threshold += 1000  # 2 * decrease_v_threshold
                    if lane_index_of_goal is not None and lane_index_of_goal >= self.lane_index:
                        change_lane2right_ver_threshold += 1000  # 2 * decrease_v_threshold
            ...
            if self.merge_tag:
                if best_lane_index > self.lane_index:  # 最佳车道在左侧
                    ...
                    change_lane2left_ver_threshold += 1000
            if len(self.raw_obs.waypoint_paths) > 1 and in_bad_lane and not self.find_all_wrong_lane_tag_init:
        ...
        if in_bad_lane and self.static_at_end_cnt >= 3 and len(
        ...
        if in_bad_lane and self.static_at_end_cnt >= 3 and len(raw_obs.waypoint_paths) == 1:
